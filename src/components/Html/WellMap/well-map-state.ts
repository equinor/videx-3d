...
export const createWellMapState = () =>
  create<WellMapState>((set, get) => ({
    _allWellbores: [],
    wellbores: [],
    wellboreIds: [],
    wellboresByName: {},
    ...
    setWellbores: wellboreHeaders => {
      ...
      const wellboresById: Record<string, WellboreHeader> =
      ...
      for (let i = 0; i < tracksOrder.length; i++) {
        ...
        if (wellbore) {
          if (
            wellbore.status === 'operating' ||
            wellbore.status === 'drilling'
          ) {
            ...
            while (
              ...
              if (activeDepths[parent.id]) {
                activeDepths[parent.id] = Math.max(
                  activeDepths[parent.id],
