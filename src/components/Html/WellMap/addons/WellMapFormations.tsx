 ...
 * @see {@link WellMap}
 */
export const WellMapFormations = ({
  stratColumnId,
}: WellMapFormationsProps) => {
  const store = useData();

  const [formationsData, setFormationsData] = useState<Record<
    string,
  ...
  useEffect(() => {
    if (store) {
      const formationsDataPromises = wellboreIds.map(id =>
        getWellboreFormations(id, stratColumnId, store),
      );
