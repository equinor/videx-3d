 ...
 * @see {@link WellMap}
 */
export const WellMapCompletionIntervals = ({ colorScreen = 'gray', colorPerforation = 'orange' }: WellMapCompletionIntervalsProps) => {

  const store = useData()

  const [screenData, setScreenData] = useState<Record<string, CompletionTool[]> | null>(null)
  const [perforationData, setPerforationData] = useState<Record<string, PerforationInterval[]> | null>(null)
  ...
  useEffect(() => {
    if (store) {
      const screenDataPromises = wellboreIds.map(id => store.get<CompletionTool[]>('completion-tools', id))
      const perforationDataPromises = wellboreIds.map(id => store.get<PerforationInterval[]>('perforations', id))

      Promise.all(screenDataPromises).then(response => {
        if (response) {
          const data = response.reduce((acc, d, i) => ({
            ...acc,
            [wellboreIds[i]]: d !== null ? d.filter(t => t.category === 'screen') : [],
          }), {})
          setScreenData(data)
        }
      })
  ...
  const intervals = useMemo(() => {
    ...
    if (screenData && perforationData) {
      wellboreIds.forEach((id) => {
        ...
        const screenSegments = screenData[id] ? toSegments(
          screenData[id].filter(d => fromMsl === undefined || d.mdBottomMsl > fromMsl),
          d => d.mdTopMsl,
          d => d.mdBottomMsl,
        ) : []
        ...
        const perforationSegments = perforationData[id] ? toSegments(
          perforationData[id].filter(d => d.status === 'Open' && (fromMsl === undefined || d.mdBottomMsl > fromMsl)),
          d => d.mdTopMsl,
          d => d.mdBottomMsl,
        ) : []
...
return output
  }, [screenData, perforationData, depthScale, getSlotPosition, slotsById, wellboreIds, wellboresById])

const getPathString = useCallback((interval: Interval) => {
  if (ratio) {
  ...
  { intervals.map(interval => (
    <g
      ...
      <path
        ...
        stroke={interval.type === 'perforation' ? colorPerforation : colorScreen}
        strokeWidth={1.5}
        fill={'none'}
      />
    </g>
