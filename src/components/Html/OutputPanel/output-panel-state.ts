...
export const useOutputPanel = () => {
  ...
  return {
    add: (id: string, group: OutputPanelGroup) => set(state => state.groups[id] ? state.groups : ({ groups: {...state.groups, [id]: group }})),
    ...
    update: (id: string, value: string | number, details?: Record<string, string | number> | null) => set(state => {
      const group = state.groups[id]
      if (group) {
        const groupDetails: OutputPanelDetails | undefined = group.details ? { ...group.details } : undefined
