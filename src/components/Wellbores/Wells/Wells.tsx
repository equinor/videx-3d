...
function calculateSegments(
  allWellbores: WellboreHeader[],
  includedWellbores: string[],
  selected?: string,
) {
  //console.time('calculateSegments')
  const idLookup = new Map<string, WellboreHeader>(
    allWellbores.map(w => [w.id, w]),
  );
  const grouped = group(allWellbores, d => d.well);
  const includedGrouped = group(
    includedWellbores
      .filter(id => idLookup.has(id))
  ...
  grouped.forEach((wellbores, well) => {
    ...
    const dictionary = wellbores.reduce(
      (dict, w) => ({ ...dict, [w.name]: w }),
      {},
    );
...
export const Wells = forwardRef<WellsRefType, WellsProps>(
  (
    ...
    const dictionary = useMemo<Record<string, WellboreHeader>>(
      () =>
        wellbores.reduce(
          (dict, wellbore) => ({ ...dict, [wellbore.id]: wellbore }),
          {},
        ),
      [wellbores],
    );
    ...
    const segments = useMemo(
      () => calculateSegments(wellbores, included, selected),
      [wellbores, included, selected],
    );
