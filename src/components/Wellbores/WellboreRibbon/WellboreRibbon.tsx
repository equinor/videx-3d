...
import {
  ...
  WellboreRibbonContextProps,
} from './WellboreRibbonContext';
...
function createStripeGeometry(
  ...
  const positions = new Float32Array([0, -0.5, 1, -0.5, 1, 0.5, 0, 0.5]);
  const from =
    fromMsl !== undefined ? trajectory.getPositionAtDepth(fromMsl, true)! : 0;
  ...
  geometry.setAttribute(
    'tangent0',
    new InterleavedBufferAttribute(buffer, 3, 4),
  );
  geometry.setAttribute(
    'tangent1',
    new InterleavedBufferAttribute(buffer, 3, 11),
  );
...
export const WellboreRibbon = ({ children }: PropsWithChildren) => {
  ...
  useEffect(() => {
    if (store) {
      store
        .get<PositionLog>('position-logs', id)
        .then(response => {
  ...
  return (
    <WellboreRibbonContext value={context}>{children}</WellboreRibbonContext>
  );
};
