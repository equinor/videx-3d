 ...
 * @expand
 */
export type PerimeterProps = CommonComponentProps &
  CustomMaterialProps & {
...
export const Perimeter = ({
  ...
  const { id, segmentsPerMeter, simplificationThreshold } =
    useWellboreContext();
  ...
  const generator = useGenerator<PerimeterGeneratorResponse>(
    perimeterGeometry,
    priority,
  );
  ...
  useEffect(() => {
    if (generator) {
      generator(id, radius, segmentsPerMeter, simplificationThreshold).then(
        response => {
          if (response) {
