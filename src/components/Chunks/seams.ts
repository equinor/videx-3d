...
export type SeamClaim = {
  ...
  rimSpacing?: number;
  /**
   * The surface is this chunk's LID — its first layer, and one that holds a
   * volume. A bare sheet lids nothing, so it does not claim the horizon.
   */
  top: boolean;
};
 ...
 * block underneath it, so drawing it with that block's own material and opacity is
 * what stops a translucent tier putting a see-through lid on an opaque one.
 * Claimants are therefore ordered lid owner first, then by area descending, then
 * by key, and each draws its footprint minus everything already taken:
 *
 ...
 *
 * A horizon that is nobody's lid has no owner, which leaves the area order: the
 * widest draws it and the others cut around it. Two identical outlines read as
 * containment, so ties stay deterministic.
 *
