...
export const GridAxesLabels = ({
  ...
  useFrame(({ camera }) => {
    ...
    if (rotations.x !== rot.x || rotations.y !== rot.y || rotations.flipped !== rot.flipped) {
      setRotations(rot)
    }
  })
  ...
  const ticks = useMemo<Ticks>(() => {
    ...
    const nTicks: Vec2 = [
      Math.min(1000, Math.floor((size[0] - (distanceToOrigin[0] % units)) / units) + 1),
      Math.min(1000, Math.floor((size[1] - (distanceToOrigin[1] % units)) / units) + 1),
    ]
