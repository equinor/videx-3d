...
const WellMapSelector = (({ colorScale }: Props) => {
  ...
  useEffect(() => {
    async function onWellboreSelect(event: WellboreSelectedEvent) {
      ...
      if (event.detail.id !== selected) {
        offsetPosition.current = wellboreManager.getInfo(event.detail.id)?.position || [0, 0, 0]
        setSelected(event.detail.id)

        const poslog = await store.get<PositionLog>('position-logs', event.detail.id)
        ...
        if (poslog) {
          ...
          const wellbore = await store.get<WellboreHeader>('wellbore-headers', event.detail.id)
          if (wellbore) {
      ...
      if (wellboreState.current !== null) {
        if (event.detail.position) {
          ...
          const nearestPosition = wellboreState.current.curve.nearest(localPosition)
          if (nearestPosition) {
            ...
            setDepth(wellboreState.current.measuredTop + nearestPosition.position * wellboreState.current.measuredLength)
  ...
  return (
    <div style={{
      ...
      {wellIdentifier && (
        <WellMap
          colors={colorScale ? wellbore => colorScale(wellbore.name.replace(wellbore.well, '') || 'Main') : undefined}
          depth={depth}
