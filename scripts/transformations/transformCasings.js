...
function mapCasingData(data) {
  ...
  return data
    .filter(
      d =>
        Number.isFinite(d.depth_top_md) &&
        (d.type === 'L' || d.type === 'C' || d.type === 'TIE'),
    )
    .map(d => ({
      type: getType(d),
        ...
        if (filtereditems.length) {
          ...
          for (let i = 0; i < filtereditems.length; i++) {
            if (filtereditems[i].type === 'L') {
              if (filtereditems[i].mdTopMsl < topMd)
                topMd = filtereditems[i].mdTopMsl;
              if (filtereditems[i].innerDiameter > maxDim)
                maxDim = filtereditems[i].innerDiameter;
            }
          }
        }
      }
