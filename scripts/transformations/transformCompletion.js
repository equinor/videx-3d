function getCategory(tool) {
  ...
  if (name.match(/(tube|tubing)/i)) {
    ...
    const match = name.match(/(blank pipe|pipe|gauge|plug|packer|injection mandrel|spm|pbr|perforation)/i)
...
function mapCompletionData(data) {
  ...
  })).filter(d => (d.diameterTop || d.diameterBottom || d.diameterMax) && d.length > 0)
}
